...
def pdf_reader(pdf_file,
               exe_file=r'C:\Program Files (x86)\tools\xpdf-tools-win-4.03\bin64\pdftotext.exe',
               xpdfrc_file =r'C:\Program Files (x86)\tools\xpdf-tools-win-4.03\xpdfrc'):
    """
    Arguments:
    ...
    print(pdf_file)
    initial_name = str(pdf_file)[:3]
    if initial_name == 'HK1':
        company_name = text_list[7]
        payment_amount = (text_list[-1]).split()[-2:]
    elif initial_name == 'MY1':
        company_name = text_list[-1]
        payment_amount = text_list[-3]
    elif initial_name == 'MY2':
        company_name = (text_list[0]).split()[:4]
        payment_amount = text_list[-1]
    elif initial_name == 'SG1':
        company_name = (text_list[0]).split()[:2]
        payment_amount = text_list[-1]
    elif initial_name == 'SG2':
        company_name = text_list[0]
        payment_amount = (text_list[-1]).split()[-2:]
    elif initial_name == 'SG3':
        company_name = (text_list[0]).split()[:3]
        payment_amount = text_list[-1]
    elif initial_name == 'SG4':
        company_name = (text_list[0]).split()[1:3]
        payment_amount = (text_list[-1]).split()[-2]
        ...
        company_name = (text_list[0]).split()[0:3]
        payment_amount = text_list[-4]
    elif initial_name == 'TW3':
        company_name = text_list[2]
        payment_amount = text_list[-1]
